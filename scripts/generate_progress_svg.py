...
AREAS = [
    ...
    ("Detección & Tracking",   date(2026, 1, 13), date(2026, 6, 13),  9,  8, "#F97316"),
    ("Geometría & Homografía", date(2026, 2, 11), date(2026, 6, 17),  4,  4, "#CA8A04"),
    ("Identidad & Equipos",    date(2026, 3, 26), date(2026, 4,  9),  3,  3, "#9333EA"),
    ("Analytics & Reglas",     date(2026, 4, 16), date(2026, 6, 18), 10,  9, "#16A34A"),
    ("Core & Infrastructure",  date(2026, 1,  8), date(2026, 6, 19), 26, 24, "#3B82F6"),
    ("Memoria TFG",            date(2026, 5, 18), date(2026, 6, 22), 13, 13, "#B45309"),
]
# ──────────────────────────────────────────────────────────────────────────

PROJECT_START = date(2026, 1, 8)
PROJECT_END   = date(2026, 6, 22)
...
deferred = total_tasks - done_tasks
stat4_val, stat4_lbl = str(deferred), "tarjetas diferidas"
...
add(
    ...
    f'Ene – Jun 2026  ·  Kanban (flujo único, WIP=1)  ·  {AUTHOR}</text>',
)
...
for i, (name, start, end, tasks, done, color) in enumerate(AREAS):
    ...
    if i % 2 == 0:
        add(f'  <rect x="0" y="{ry-1}" width="{SVG_W}" height="{ROW_H+2}" fill="#000000" opacity="0.04"/>')
    ...
    # Etiqueta done/tasks: fuera a la derecha, o dentro del extremo si no cabe
    ty = ry + ROW_H // 2 + 4
    if x2 > SVG_W - 44:
        add(f'  <text x="{x2-8:.1f}" y="{ty}" font-size="10" text-anchor="end"'
            f' fill="#ffffff" font-weight="600">{done}/{tasks}</text>')
    else:
        add(f'  <text x="{x2+6:.1f}" y="{ty}" font-size="10" fill="#999999">{done}/{tasks}</text>')

# Marcador de entrega ordinaria (18 may)
ox = xp(ORDINARIA)
add(
    f'  <line x1="{ox:.1f}" y1="{AXIS_Y-2}" x2="{ox:.1f}" y2="{TOP+GANTT_H-4}"'
    f' stroke="#F59E0B" stroke-width="1.3" stroke-dasharray="5,4"/>',
    f'  <text x="{ox-3:.1f}" y="{AXIS_Y}" font-size="9" text-anchor="end"'
    f' fill="#B45309" font-weight="600">Ordinaria</text>',
)

# Línea de entrega final
dlx = xp(PROJECT_END)
add(
    ...
    f' fill="#EF4444" font-weight="600">Final</text>',
)

# Leyenda — 2 filas de 4
leg_items = [(name, color) for name, *_, color in AREAS] + [("Entrega final (22 jun)", "#EF4444")]
for row in range(2):
    lx = 20
    ly = LEG_Y1 + row * 18
    for name, color in leg_items[row*4:(row+1)*4]:
        if name == "Entrega final (22 jun)":
